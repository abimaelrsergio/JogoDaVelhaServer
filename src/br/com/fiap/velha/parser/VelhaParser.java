package br.com.fiap.velha.parser;
...
public class VelhaParser {
	 ...
	 */
	public Velha getVelhaBean(String xmlContent) {
		...
		char id = MotorDoJogoDaVelha.JOGADOR_VAZIO;
		if ("X".equals(idStr))
			id = MotorDoJogoDaVelha.JOGADOR_X;
		else if ("O".equals(idStr))
			id = MotorDoJogoDaVelha.JOGADOR_O;
		velhaBean.setId(id);
		...
		for (int pos = 0; pos < NODE_TAB_POS.length; pos++) {
			char jogador = MotorDoJogoDaVelha.JOGADOR_VAZIO;
			...
			if (posStr != null && !posStr.isEmpty()) {
				...
				if (posStr.equals("X"))
					jogador = MotorDoJogoDaVelha.JOGADOR_X;
				else if (posStr.equals("O"))
					jogador = MotorDoJogoDaVelha.JOGADOR_O;
			}
	 ...
	 */
	public String getVelhaXML(Velha velhaBean) {
        ...
        for (int pos = 0; pos < NODE_TAB_POS.length; pos++) {
        	...
        	if (velhaBean.getPosicao(pos) != MotorDoJogoDaVelha.JOGADOR_VAZIO) {
        		textValue =
        			doc.createTextNode(
